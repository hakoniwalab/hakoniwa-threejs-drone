...
export class FleetStateSource extends IStateSource {
  ...
  collectChannelsByPdutype(robots, pdutype) {
    const channels = [];
    for (const robot of robots) {
      ...
      const scan = (arr) => {
        ...
        for (const pdu of arr) {
          ...
          channels.push({
            robotName,
            pduName: pdu.org_name,
          });
        }
      };
      scan(robot?.shm_pdu_readers);
      scan(robot?.shm_pdu_writers);
    }
    return channels;
  }
  ...
  async initialize({ pduDefPath } = {}) {
    ...
    const channels = this.collectChannelsByPdutype(robots, visualStatePdutype);
